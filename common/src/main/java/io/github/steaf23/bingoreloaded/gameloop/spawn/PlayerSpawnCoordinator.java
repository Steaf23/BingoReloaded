...
public class PlayerSpawnCoordinator {
	...
	public PlayerSpawnCoordinator(PlatformServer server, SpawnStrategy strategy, int maxTeleportDistance) {
	...
	public void teleportPlayersToStart(BingoSession session, TeamContainer teams, int platformLifetime) {
		...
		for (SpawnStrategy.SpawnSite site : sites) {
			...
			BingoGame.spawnPlatform(server.getWorld(site.position().dimension()), site.position(), 5, true);
			server.taskScheduler().runTask(platformLifetime, _ ->
					BingoGame.removePlatform(server.getWorld(site.position().dimension()), site.position(), 5));
		}
	}

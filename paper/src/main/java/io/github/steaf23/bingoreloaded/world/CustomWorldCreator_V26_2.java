...
public class CustomWorldCreator_V26_2 {
	...
	public static @Nullable World createBingoWorld(@NotNull Key worldKey, @Nullable Key noiseSettingsLocation) {
		...
		final ResourceKey<net.minecraft.world.level.Level> dimensionKey = CraftNamespacedKey.toResourceKey(Registries.DIMENSION, new NamespacedKey(worldKey.namespace(), worldKey.value()));
		...
		// ---------- Bingo Reloaded ---- Removed migration code
		...
		LevelStem customStem = createCustomStem(levelStemRegistry, actualDimension, noiseSettingsLocation, console);
		if (customStem == null) {
			ConsoleMessenger.bug("Could not create a bingo world with smaller biomes", CustomWorldCreator_V26_2.class);

...
public class BingoCardHudElement implements HudElement {
	...
	private static final Identifier GAMEMODE_LOGOS = Identifier.parse("bingoreloadedcompanion:textures/gui/gamemode_logos.png");
	private static final Identifier TASK_BACKGROUND = Identifier.parse("bingoreloadedcompanion:textures/gui/sprites/task_background.png");
	private static final Identifier TASK_BACKGROUND_ADVANCEMENT = Identifier.parse("bingoreloadedcompanion:textures/gui/sprites/task_background_advancement.png");
	private static final Identifier TASK_BACKGROUND_STATISTIC = Identifier.parse("bingoreloadedcompanion:textures/gui/sprites/task_background_statistic.png");
	private static final Identifier TASK_BACKGROUND_HOTSWAP = Identifier.parse("bingoreloadedcompanion:textures/gui/sprites/task_background_hotswap.png");
	private static final Identifier TASK_ADVANCEMENT_ICON = Identifier.parse("bingoreloadedcompanion:textures/gui/sprites/task_advancement_icon.png");
	...
	private static final TextColorGradient HOTSWAP_EXPIRATION_GRADIENT = new TextColorGradient()
			...
			.addColor(TextColor.parseColor("#3cff00").getOrThrow(), 0.05f)
			.addColor(TextColor.parseColor("#ffd200").getOrThrow(), 0.25f)
			.addColor(TextColor.parseColor("#e85e21").getOrThrow(), 0.5f)
			.addColor(TextColor.parseColor("#750e0e").getOrThrow(), 0.8f)
			.addColor(TextColor.fromLegacyFormat(ChatFormatting.DARK_GRAY), 1.0f);
	...
	public void render(GuiGraphics drawContext, DeltaTracker renderTickCounter) {
		...
		renderElement(drawContext, renderTickCounter.getGameTimeDeltaTicks());
	}
	...
	private void renderTasks(GuiGraphics context, float tickDelta) {
		...
		float scaleFactorX = 1.0f / Minecraft.getInstance().getWindow().getGuiScale() * placement.scaleX();
		float scaleFactorY = 1.0f / Minecraft.getInstance().getWindow().getGuiScale() * placement.scaleY();
	...
	protected void renderTask(GuiGraphics drawContext, @NotNull Task task, int x, int y, @Nullable HotswapTaskHolder hotswapContext, float delta, double transparency) {
		...
		if (hotswapRecovering) {
			...
			drawContext.blit(RenderPipelines.GUI_TEXTURED, backgroundTexture, borderX, borderY, 0, 0, 21, 21, 21, 21, ScreenHelper.addAlphaToColor(0xFFFFFF, (int)(255 * transparency)));
		}

		// Completion overlay
		if (completion.completed()) {
			drawContext.blit(RenderPipelines.GUI_TEXTURED, TASK_COMPLETED_OVERLAY, borderX, borderY, 0, 0, 21, 21, 21, 21, ScreenHelper.addAlphaToColor(completion.teamColor(), 255));
		}
	...
	private void renderHotswapBackground(GuiGraphics context, float startTime, float currentTime, int x, int y, int color, boolean reverse) {
		...
		context.blit(RenderPipelines.GUI_TEXTURED, TASK_BACKGROUND_HOTSWAP, x, y, currentFrame * frameWidth, frameHeight, frameWidth, frameHeight, frameWidth * totalFrames, frameHeight, color);
	}
}
